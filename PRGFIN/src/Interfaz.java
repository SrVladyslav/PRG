...
public class Interfaz extends JFrame{
	...
	protected class Panel extends JPanel{
		...
		/**BUTTONS**/
		protected JButton search, add, delete,voice, text;
		/**VOICE**/
		protected boolean activeVoice = true, activeInformation = true, activeText = true;
		...
		public Panel() {
			...
			text.setBounds(450,positionYButtons + 70,300,50);
			text.setForeground(blueColor);
			text.setFont(new Font("Calibri",Font.ITALIC, 20));
			text.addActionListener(new Raton());
		...
		protected class Raton extends KeyAdapter implements ActionListener ,MouseListener{
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(e.getSource().equals(search)) {			
					...
					/**WINDOW**/
					ShowStringEst manu = new ShowStringEst();;
					if(activeText) {
			...
			/**Window for text traduction**/
			protected class ShowStringEst extends JFrame{
				public ShowStringEst() {
					setLayout(null);
					setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
					setResizable(false);
					setBounds(300,300,1000,500);
					setTitle("Dictionary Est.");
					setIconImage(Interfaz.icono);
					PanToString p = new PanToString();
					JScrollPane scrollBar = new JScrollPane(p,
				            JScrollPane.VERTICAL_SCROLLBAR_ALWAYS,
				...
				private class PanToString extends JPanel{
					public PanToString() {
						...
						text.setFont(new Font("Consol",Font.ITALIC,20));
						try{
							File f = new File("src/datos/texto.txt");
							Diccionario d = new Diccionario(f);
							text.setText(d.toStringEst());
						}catch(Exception e) {System.out.println("YEP"); e.getMessage(); e.printStackTrace();}
						setPreferredSize(new Dimension(1000,5000));
						text.setVisible(true);
						add(text);
					}
